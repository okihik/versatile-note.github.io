...
*   $I_{t-1}$ = Inventory at the end of period $t-1$
*   $P_{t}$ = Production in period $t$
...
Where:
*   $I_{target}$ = Desired target inventory level
*   $k$ = A control parameter (e.g., a fraction of the inventory deviation to correct in the next period)
...
Where:
*   $WIP_{t}$ = Work-in-process at the end of period $t$
*   $WIP_{max}$ = Maximum allowable work-in-process
